...
awk 'BEGIN{OFS="#"{print $1, "HELLO",$2, $3,}}'  /etc/passwd
...
2.2 awk内置变量之数据变量：
NR: The number of input records，awk 理的记录数；如果有多个文件，这个数目会把处理的多个文件中行统一计数；
...
awk -F： '{shell[$NF]++} END {for(str in shell) print str, shell[str]}'
...
```
---
**备份地址**: 【[awk](https://coderr.blog.csdn.net/article/details/62443439)】
